...
((progress.style.strokeDasharray = circumference),
  ...
  window.addEventListener("scroll", () => {
    ...
    e > 1 && (e = 1);
    const t = Math.max(0, circumference * (1 - e));    progress.style.strokeDashoffset = t;
  }));
var tag = document.createElement("script");
tag.src = "https://www.youtube.com/iframe_api";
var firstScriptTag = document.getElementsByTagName("script")[0];
function hmsToSeconds(e) {
  for (var t = e.split(":"), r = 0, n = 1; t.length > 0; )
    ...
    t.forEach((e) => {
      ...
      (e.addEventListener("dragover", (e) => e.preventDefault()),
        e.addEventListener("drop", (r) => {
          if (
            (r.preventDefault(),
            e.closest(".game-step").classList.contains("reveal-all"))
          )
            return;
...
(verifyBtn.addEventListener("click", () => {
  ...
  if (
    ...
    return (
      (feedback.innerText =
        "Veuillez remplir toutes les cases avant de valider !"),
      void (feedback.style.color = "orange")
    );
  (e.classList.add("reveal-all"),
    t.forEach((e) => {
      const t = e.querySelector(".tag");
      (e.classList.add("reveal"),
        t.dataset.style === e.dataset.answer
    ...
    (feedback.innerText =
      "Découvrez les explications et le complément d'info !"),
  ...
  nextBtn.addEventListener("click", () => {
    currentStep < 3
      ? (document
        ...
        setupDragAndDrop())
      : (feedback.innerText =
          "Félicitations, vous avez fini Behind The Sound !");
  }));
const progress = document.getElementById("progress"),
    radius = progress.r.baseVal.value,
    circumference = 2 * Math.PI * radius;
